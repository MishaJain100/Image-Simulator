...
class AutotuningAndCalibrationLogic(QtWidgets.QMainWindow):
    ...
    def reset(self):
        ...
        if self.tuning_thread and self.tuning_thread.isRunning():
            self.tuning_thread.terminate()
            self.tuning_thread.wait()
    ...
    def on_tuning_finished(self, estimated_params):
        ...
        if 'error' in estimated_params:
            QtWidgets.QMessageBox.critical(self, "Tuning Failed", f"Could not tune parameters.\nError: {estimated_params['error']}")
            self.ui.AutoTuningProgress.setText("Tuning Failed")
            return
        ...
        dist = self.estimated_params.get('distortion', np.zeros(5))
        noise = self.estimated_params.get('noise', 0.0) 
        sw = self.estimated_params.get('sensor_width', 36.0)
        sh = self.estimated_params.get('sensor_height', 24.0)
        fl = self.estimated_params.get('focal_length', 50.0)
        loss = self.estimated_params.get('final_loss', 0.0)

        dist_str = ", ".join([f"{d:.3f}" for d in dist])
        self.ui.DistortionNumber.setText(f"[{dist_str}]")
        
        self.ui.NoiseNumber.setText(f"{noise:.2f}")
        self.ui.FLNumber.setText(f"{fl:.2f} mm")
        self.ui.SSNumber.setText(f"{sw:.1f} x {sh:.1f} mm")

        self.ui.AutoTuningProgress.setText(f"Complete (Error: {loss:.4f})")

    def apply_parameters(self):
        if not self.estimated_params: 
            return
        print("Parameters ready to apply:", self.estimated_params)